    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 100 || requestCode == 101 && resultCode == RESULT_OK) {
            if (mTmpFile.length() > 0) {
                MediaBean mediaBean = new MediaBean(mTmpFile.getPath(), mTmpFile.getName(), 0, 1, mTmpFile.length(), 0, "");
                mediaBeans.add(mediaBean);
                Intent intent = new Intent();
                intent.putParcelableArrayListExtra(PickerConfig.EXTRA_RESULT, mediaBeans);
