...
package com.onlyknow.app.utils.zxing.camera;
...
public final class OKCameraManager {

	private static final String TAG = OKCameraManager.class.getSimpleName();

	private static final int MIN_FRAME_WIDTH = 240;
	private static final int MIN_FRAME_HEIGHT = 240;
	private static final int MAX_FRAME_WIDTH = 480 * 2;
	private static final int MAX_FRAME_HEIGHT = 360 * 2;

	private static OKCameraManager cameraManager;
	...
	private final OKCameraConfigurationManager configManager;
	 ...
	 * message.
	 */
	private final OKPreviewCallback previewCallback;
	/**
	 * Autofocus callbacks arrive here, and are dispatched to the Handler which
	 * requested them.
	 */
	private final OKAutoFocusCallback autoFocusCallback;
	...
	public static void init(Context context) {
		if (cameraManager == null) {
			cameraManager = new OKCameraManager(context);
		}
	}

	/**
	 * Gets the OKCameraManager singleton instance.
	 * 
	 * @return A reference to the OKCameraManager singleton.
	 */
	public static OKCameraManager get() {
	...
	private OKCameraManager(Context context) {
		...
		this.configManager = new OKCameraConfigurationManager(context);
		...
		previewCallback = new OKPreviewCallback(configManager, useOneShotPreviewCallback);
		autoFocusCallback = new OKAutoFocusCallback();
	}
	...
	public void openDriver(SurfaceHolder holder) throws IOException {
		if (camera == null) {
			...
			OKFlashlightManager.enableFlashlight();
		}
	}
	...
	public void closeDriver() {
		if (camera != null) {
			OKFlashlightManager.disableFlashlight();
			camera.release();
			camera = null;
		}
	}
	 ...
	 * @return A OKPlanarYUVLuminanceSource instance.
	 */
	public OKPlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height) {
		...
		case PixelFormat.YCbCr_422_SP:
			return new OKPlanarYUVLuminanceSource(data, width, height, rect.left, rect.top, rect.width(), rect.height());
		default:
			...
			if ("yuv420p".equals(previewFormatString)) {
				return new OKPlanarYUVLuminanceSource(data, width, height, rect.left, rect.top, rect.width(),
						rect.height());
			}
		}
