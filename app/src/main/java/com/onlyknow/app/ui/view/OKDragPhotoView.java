...
public class OKDragPhotoView extends PhotoView {
    ...
    public OKDragPhotoView(Context context) {
        this(context, null);
    }

    public OKDragPhotoView(Context context, AttributeSet attr) {
        this(context, attr, 0);
    }

    public OKDragPhotoView(Context context, AttributeSet attr, int defStyle) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (event.getPointerCount() == 1) {
                        onActionUp(event);
                        isTouchEvent = false;
                        //judge loadWeatherComplete or not
                            ...
                            public void run() {
                                if (mTranslateX == 0 && mTranslateY == 0 && canFinish) {
                                    ...
                                    if (mTapListener != null) {
                                        mTapListener.onTap(OKDragPhotoView.this);
                                    }
                                }
                                canFinish = false;
                            }
    ...
    private void onActionUp(MotionEvent event) {
        ...
        if (mTranslateY > MAX_TRANSLATE_Y) {
            if (mExitListener != null) {
                ...
                throw new RuntimeException("OKDragPhotoView: onExitLister can't be null ! call setOnExitListener() ");
            }
        } else {
            performAnimation();
        }
    ...
    public interface OnTapListener {
        void onTap(OKDragPhotoView view);
    }

    public interface OnExitListener {
        void onExit(OKDragPhotoView view, float translateX, float translateY, float w, float h);
    }
