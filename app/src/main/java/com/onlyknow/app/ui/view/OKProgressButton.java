...
public class OKProgressButton extends TextView {
    ...
    public OKProgressButton(Context context) {
        this(context, null);

    }

    public OKProgressButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        if (!isInEditMode()) {
            mContext = context;
            initController();
    ...
    public void setState(int state) {
        if (mState != state) {//状态确实有改变
            this.mState = state;
            invalidate();
            if (state == OKProgressButton.INSTALLING) {
                //开启两个点动画
    ...
    public OKProgressButton setCustomerController(ButtonController customerController) {
    ...
    class DefaultButtonController implements ButtonController {
