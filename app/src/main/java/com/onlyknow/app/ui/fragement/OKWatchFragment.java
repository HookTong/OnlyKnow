...
public class OKWatchFragment extends OKBaseFragment implements OnRefreshListener, OnLoadMoreListener {
    private RefreshLayout mRefreshLayout;
    private OKRecyclerView mOKRecyclerView;
    private CardViewAdapter mCardViewAdapter;

    private OKLoadWatchApi mOKLoadWatchApi;
    ...
    private OKLoadWatchApi.onCallBack mOnCallBack = new OKLoadWatchApi.onCallBack() {
        @Override
        public void cardList(List<OKCardBean> list) {
            if (list != null) {
                ...
                OKConstant.putListCache(INTERFACE_COLLECTION, mCardBeanList);
    ...
    public void onLoadMore(RefreshLayout refreshLayout) {
        ...
        if (USER_INFO_SP.getBoolean("STATE", false)) {
            ...
            mOKLoadWatchApi.requestCardBeanList(map, true, mOnCallBack);
        } else {
            mRefreshLayout.finishLoadMore(1500);
    ...
    public void onRefresh(RefreshLayout refreshLayout) {
        if (!OKNetUtil.isNet(getActivity())) {
            if (USER_INFO_SP.getBoolean("STATE", false) && mOKRecyclerView.getAdapter().getItemCount() == 0) {
                mCardBeanList.clear();
                mCardBeanList.addAll(OKConstant.getListCache(INTERFACE_COLLECTION));
                mOKRecyclerView.getAdapter().notifyDataSetChanged();
            }
        ...
        if (USER_INFO_SP.getBoolean("STATE", false)) {
            Map<String, String> map = new HashMap<>();// 请求参数
            map.put("username", USER_INFO_SP.getString(OKUserInfoBean.KEY_USERNAME, ""));
            map.put("num", OKConstant.WATCH_LOAD_COUNT);
            if (mOKLoadWatchApi == null) {
                mOKLoadWatchApi = new OKLoadWatchApi(getActivity(), false);
            }
            mOKLoadWatchApi.requestCardBeanList(map, false, mOnCallBack);
        } else {
            mRefreshLayout.finishRefresh(1500);
    ...
    private class CardViewAdapter extends RecyclerView.Adapter<CardViewAdapter.CardViewHolder> {
        ...
        private void initViews(final CardViewHolder mCardViewHolder, final OKCardBean okCardBean, final int position) {
            ...
            mCardViewHolder.mImageViewDelete.setOnClickListener(new OnClickListener() {
                ...
                @Override
                public void onClick(View v) {
                    showAlertDialog("删除收藏", "是否删除该条收藏 ?", "确定", "取消", new DialogInterface.OnClickListener() {
                        @Override
        ...
        public void removeCardBean(int i) {
            ...
            mBeanList.remove(i);
            OKConstant.removeListCache(INTERFACE_COLLECTION, i);
            mOKRecyclerView.getAdapter().notifyDataSetChanged();
        }
